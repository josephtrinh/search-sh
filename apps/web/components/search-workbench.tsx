...
export function SearchWorkbench() {
  ...
  const [mode, setMode] = useState("auto"); const [visualModel, setVisualModel] = useState<VisualModel>("siglip2"); const [pending, startTransition] = useTransition(); const inputRef = useRef<HTMLInputElement>(null); const requestRef = useRef(0);
  ...
  async function run(cursor?: string, filters = selected) {
    const requestId = ++requestRef.current; setError(null); const form = new FormData(); if (query.trim()) form.set("query", query.trim()); if (image) form.set("image", image); form.set("mode", mode);
    form.set("filters", JSON.stringify(filters)); form.set("limit", "24"); if (cursor) form.set("cursor", cursor);
    try { const response = await fetch(`${API}/search`, { method: "POST", body: form }); if (!response.ok) throw new Error((await response.json()).message ?? "Search failed");
      const next = await response.json() as SearchResponse; if (requestId !== requestRef.current) return; setVisualModel(next.visualModel); setResult((previous) => {
    ...
    } catch (cause) { if (requestId === requestRef.current) setError(cause instanceof Error ? cause.message : "Search failed"); }
  }
  function submit(event: React.FormEvent) { event.preventDefault(); startTransition(() => void run()); }
  function toggle(key: string, value: string) {
    const values = selected[key] ?? []; const nextValues = values.includes(value) ? values.filter((item) => item !== value) : [...values, value];
    const next = { ...selected, [key]: nextValues }; if (nextValues.length === 0) delete next[key]; setSelected(next);
    if (result) startTransition(() => void run(undefined, next));
  }
  const hasSelectedFilters = Object.values(selected).some((values) => values.length > 0);
  function clearFilters() { if (!hasSelectedFilters) return; setSelected({}); if (result) startTransition(() => void run(undefined, {})); }
  ...
  return <section className="workbench">
    ...
    <div className="results-layout"><aside className="facets"><div className="aside-title"><strong>Refine</strong><button type="button" disabled={!hasSelectedFilters} onClick={clearFilters}>Clear</button></div>
