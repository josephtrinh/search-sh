...
export function AdminDashboard() {
  ...
  const [captions, setCaptions] = useState<CaptionProviderStatus | null>(null);
  const [previewLimit, setPreviewLimit] = useState(10_000);
  const [error, setError] = useState<string | null>(null);
  const [pending, startTransition] = useTransition();

  async function refresh() {
    const [runsResponse, rankingResponse, healthResponse, visualResponse, scopeResponse, captionResponse] = await Promise.all([
      fetch(`${API}/admin/index-runs`), fetch(`${API}/admin/ranking`), fetch(`${API}/health`), fetch(`${API}/admin/visual-model`), fetch(`${API}/admin/index-scope`), fetch(`${API}/admin/caption-provider`),
    ]);
    ...
    setCaptions(await captionResponse.json());
  }
  useEffect(() => { void refresh(); const timer = window.setInterval(() => void refresh(), 5000); return () => window.clearInterval(timer); }, []);

  async function start(mode: IndexRunMode, generation?: VisualGeneration, captionProvider?: CaptionProvider) {
    setError(null);
    const response = await fetch(`${API}/admin/index-runs`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mode, ...(mode === "limited_full" ? { generation, productLimit: previewLimit } : {}), ...(mode === "caption_backfill" ? { captionProvider, targetScope: scope?.active ?? "stable" } : {}) }) });
  ...
  async function selectCaptionProvider(provider: CaptionProvider) {
    setError(null);
    const response = await fetch(`${API}/admin/caption-provider`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ provider }) });
    if (!response.ok) throw new Error((await response.json()).message ?? "Unable to switch caption provider");
  ...
  return <div className="admin-grid">
    ...
    <section className="panel index-panel"><div className="panel-heading"><div><p className="kicker">INDEX</p><h2>Catalog operations</h2></div><div className="button-pair"><button disabled={pending} onClick={() => perform(() => start("incremental"))}>Incremental</button><button disabled={pending} onClick={() => perform(() => start("visual_backfill"))}>Backfill DINOv2</button><button disabled={pending} onClick={() => perform(() => start("dinov3_backfill"))}>Backfill DINOv3</button><button disabled={pending} className="primary" onClick={() => perform(() => start("full"))}>Full rebuild</button></div></div>
      ...
      <div className="visual-switch"><div><strong>Active caption provider</strong><small>Stored caption and E5 vectors switch instantly for the active {scope?.active.replaceAll("_", " ") ?? "index"}. Backfills update only the selected provider in that scope.</small></div><div className="button-pair"><button className={captions?.active === "florence" ? "active" : ""} disabled={pending || !captions?.florenceReady} onClick={() => perform(() => selectCaptionProvider("florence"))}>Florence{captions?.florenceReady ? "" : " · not ready"}</button><button className={captions?.active === "qwen" ? "active" : ""} disabled={pending || !captions?.qwenReady} onClick={() => perform(() => selectCaptionProvider("qwen"))}>Qwen{captions?.qwenReady ? "" : " · not ready"}</button><button disabled={pending} onClick={() => perform(() => start("caption_backfill", undefined, "florence"))}>Backfill Florence</button><button disabled={pending} onClick={() => perform(() => start("caption_backfill", undefined, "qwen"))}>Backfill Qwen</button></div></div>
      <div className="run-list">{runs.length ? runs.slice(0, 6).map((run) => <div className="run" key={run.id}><span className={`run-state ${run.status}`}>{run.status}</span><div><strong>{run.mode.replaceAll("_", " ")} index{run.visualGeneration ? ` · ${run.visualGeneration}` : ""}{run.captionProvider ? ` · ${run.captionProvider} · ${run.targetScope?.replaceAll("_", " ")}` : ""}</strong><small>{run.mode === "caption_backfill" && run.status === "running" && run.processedProducts === 0 ? `Preparing ${run.captionProvider ?? "Florence"} captions and E5 vectors…` : <>{run.processedProducts.toLocaleString()} / {run.totalProducts.toLocaleString()} products · coverage {coverageSummary(run)} · {run.normalizedImages.toLocaleString()} normalized · {run.rejectedSourceImages.toLocaleString()} rejected · {run.failedCaptions.toLocaleString()} caption failures</>}</small>{run.qualityWarning ? <small className="coverage-warning">{run.qualityWarning}</small> : null}</div><progress value={run.processedProducts} max={Math.max(run.totalProducts, 1)} />{["queued", "running", "cancelling"].includes(run.status) ? <button className="cancel-run" disabled={pending || run.status === "cancelling"} onClick={() => perform(() => cancel(run.id))}>{run.status === "cancelling" ? "Cancelling…" : "Cancel"}</button> : <span />}</div>) : <p>No indexing runs yet.</p>}</div></section>
