...
export function AdminDashboard() {
  ...
  return <div className="admin-grid">
    ...
    <section className="panel index-panel"><div className="panel-heading"><div><p className="kicker">INDEX</p><h2>Catalog operations</h2></div><div className="button-pair"><button disabled={pending} onClick={() => perform(() => start("incremental"))}>Incremental</button><button disabled={pending} onClick={() => perform(() => start("visual_backfill"))}>Backfill DINOv2</button><button disabled={pending} onClick={() => perform(() => start("dinov3_backfill"))}>Backfill DINOv3</button><button disabled={pending} className="primary" onClick={() => perform(() => start("full"))}>Full rebuild</button></div></div>
      ...
      <div className="run-list">{runs.length ? runs.slice(0, 6).map((run) => <div className="run" key={run.id}><span className={`run-state ${run.status}`}>{run.status}</span><div><strong>{run.mode.replaceAll("_", " ")} index</strong><small>{["visual_backfill", "dinov3_backfill"].includes(run.mode) && run.status === "running" && run.processedProducts === 0 ? `Preparing ${run.mode === "dinov3_backfill" ? "DINOv3" : "DINOv2"} model and Meilisearch vectors…` : <>{run.processedProducts.toLocaleString()} / {run.totalProducts.toLocaleString()} products · SigLIP {run.siglipEmbeddedImages.toLocaleString()} / {run.siglipFailedImages.toLocaleString()} failed · DINOv2 {run.dinov2EmbeddedImages.toLocaleString()} / {run.dinov2FailedImages.toLocaleString()} failed · DINOv3 {run.dinov3EmbeddedImages.toLocaleString()} / {run.dinov3FailedImages.toLocaleString()} failed · {run.captionedImages.toLocaleString()} captions · {run.cachedCaptions.toLocaleString()} cached</>}</small></div><progress value={run.processedProducts} max={Math.max(run.totalProducts, 1)} /></div>) : <p>No indexing runs yet.</p>}</div></section>
